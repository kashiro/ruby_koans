...
class AboutRegularExpressions < Neo::Koan
  ...
  def test_question_mark_means_optional
    ...
    assert_equal "a", "abbcccddddeeeee"[/az?/] #謎な挙動
  end
  ...
  def test_the_left_most_match_wins
    assert_equal "a", "abbccc az"[/az*/] # 謎な挙動
  end
  ...
  def test_sub_is_like_find_and_replace
    assert_equal "one t-three", "one two-three".sub(/(t\w*)/) { $1[0, 1] } #謎な挙動
  end

  def test_gsub_is_like_find_and_replace_all
    assert_equal "one t-t", "one two-three".gsub(/(t\w*)/) { $1[0, 1] } #謎な挙動
  end
end
